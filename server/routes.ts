...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Conditional admin auth middleware for configuration endpoints
  // Allows unauthenticated access only during first-run setup
  const requireAdminOrFirstRun = async (req: any, res: any, next: any) => {
    try {
      const existingConfig = await storage.getSystemConfig();
      
      // If no config exists, allow unauthenticated access for first-run setup
      if (!existingConfig) {
        return next();
      }
      
      // If config exists, require admin authentication
      return isAdmin(req, res, next);
    } catch (error) {
      console.error("Error checking config status for auth:", error);
      res.status(500).json({ message: "Failed to verify authentication requirements" });
    }
  };
  ...
  app.post('/api/system/config', requireAdminOrFirstRun, async (req, res) => {
    try {
  ...
  app.put('/api/system/config', requireAdminOrFirstRun, async (req, res) => {
    try {
      const updates = insertSystemConfigSchema.partial().parse(req.body);
  ...
  app.post('/api/system/config/reset', isAdmin, async (req, res) => {
    try {
      // Reset system configuration to initial state
      const resetConfig = {
        setupComplete: false,
        mode: 'auto' as const,
        enabledModules: [],
      ...
      const config = await storage.updateSystemConfig(resetConfig);
      res.json(config);
    } catch (error) {
      console.error("Error resetting system config:", error);
      res.status(500).json({ message: "Failed to reset system configuration" });
    }
  });

  // Network discovery routes
  app.post('/api/system/discovery/start', async (req, res) => {
    try {
      // This would integrate with the existing network discovery service
      // For now, return success - implementation would depend on the networking layer
      res.json({ success: true, message: "Network discovery started" });
    } catch (error) {
      console.error("Error starting network discovery:", error);
      res.status(500).json({ message: "Failed to start network discovery" });
    }
  });

  app.post('/api/system/discovery/stop', async (req, res) => {
    try {
      // Stop network discovery service
      res.json({ success: true, message: "Network discovery stopped" });
    } catch (error) {
      console.error("Error stopping network discovery:", error);
      res.status(500).json({ message: "Failed to stop network discovery" });
    }
  });

  app.get('/api/system/discovery/servers', async (req, res) => {
    try {
      // Return discovered servers - this would integrate with the network discovery service
      const mockServers = [
        {
          id: 'server-1',
      ...
      res.status(500).json({ message: "Failed to fetch discovered servers" });
    }
  });
  ...
  app.post('/api/system/test-connection', async (req, res) => {
    try {
      ...
      if (!address || !port) {
        return res.status(400).json({ message: "Address and port are required" });
      }

      // Test connection - this is a simplified implementation
      // In real implementation, this would use the network utility functions
      const startTime = Date.now();
      
      try {
        // Simple fetch test to the target server
